...
def chat_view(request):
    ...
    if request.method == 'POST':
        ...
        if content:
            Message.objects.create(user=user, role='user', content=content)

            history = Message.objects.filter(user=user).order_by('created_at')
            messages_for_ai = [{"role": m.role, "content": m.content} for m in history]

            ai_reply = call_ai_agent(messages_for_ai, user.id)

            Message.objects.create(user=user, role='assistant', content=ai_reply)
